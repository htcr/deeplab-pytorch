...
def train(config_path, cuda):
    ...
    # Model setup
    model = DeepLabV2JointBKS(n_classes=CONFIG.MODEL.N_CLASSES, n_blocks=[3, 4, 23, 3], atrous_rates=[6, 12, 18, 24])
    ...
    variant_name = "ablation_baseline"
    ...
    for iteration in tqdm(
        ...
        try:
            _, true_fgs, comp_fgs, bgs, labels, is_real_flags = next(loader_iter)
        except:
            loader_iter = iter(loader)
            _, true_fgs, comp_fgs, bgs, labels, is_real_flags = next(loader_iter)
        ...
        seg_loss_raw = F.cross_entropy(logits, seg_labels.to(device), 
        ...
        iter_loss = seg_loss + 3.0*mat_loss
