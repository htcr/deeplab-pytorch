...
class HumanInDome(Dataset):
    def __init__(self, CONFIG):
        ...
        self.bg_drop_rate = CONFIG.BG_DROP_RATE
        self.bg_drop_max_brightness = CONFIG.BG_DROP_MAX_BRIGHTNESS
    ...
    def __getitem__(self, index):
        ...
        if toss > self.syn_ratio:
            ...
            real_bg = real_bg.astype(np.float32)
            if np.random.rand() < self.bg_drop_rate:
                real_bg *= (self.bg_drop_max_brightness * np.random.rand())
...
def test():
    ...
    CONFIG = Dict(yaml.load(open('/home/mscv1/Desktop/FRL/IntelliThresh/masker/deeplab_pytorch/configs/jointbksv2_dropbg.yaml', 'r')))
