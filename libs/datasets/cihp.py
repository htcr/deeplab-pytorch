...
class CIHP(_BaseDataset):
    ...
    def _set_files(self):
        ...
        if self.split in ["train", "trainval", "val"]:
            self.files = list()
            self.labels = list()
            for spt, folder in [("train", "Training"), ("val", "Validation")]:
                if spt in self.split: 
                    list_txt_path = osp.join(self.root, folder, spt + "_id.txt")
                    with open(list_txt_path) as f:
                        id_list = f.readlines()
                        img_list = [osp.join(folder, "Images", img_id.rstrip() + ".jpg") for img_id in id_list]
                        label_list = [osp.join(folder, "Category_ids", img_id.rstrip() + ".png") for img_id in id_list]
                        self.files += img_list
